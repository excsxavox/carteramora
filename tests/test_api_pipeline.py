...
def test_normalizar_fecha_iso():
    assert normalizar_fecha_corte("2026-04-05") == "05042026"


def test_normalizar_fecha_ddmmyyyy():
    assert normalizar_fecha_corte("05042026") == "05042026"
...
def test_post_pipeline_ok(mock_ejecutar):
    ...
    response = client.post("/pipeline", json={"fecha": "2026-04-05"})

    assert response.status_code == 200
    data = response.json()
    assert data["ok"] is True
    assert data["fecha_corte"] == "05042026"
